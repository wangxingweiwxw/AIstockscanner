...
st.set_page_config(
    page_title="AI Stock Scanner - Enhanced",
    page_icon="📈",
    layout="wide",
...
def ai_stock_scanner():
    st.title("📈 AI Stock Scanner - Enhanced Version")
    st.markdown("---")
    ...
    with col1:
        ...
        single_stock_code = st.text_input(
            "输入单个股票代码:",
            placeholder="A股: 000001, 港股: 00700.HK, 美股: AAPL"
        )
    ...
    with col2:
        ...
        batch_stock_codes = st.text_input(
            "输入多个股票代码 (逗号分隔):",
            placeholder="000001,600519,00700.HK,AAPL"
        )
    ...
    all_stock_codes = list(set(all_stock_codes))[:5]
            ...
            if analysis_type == "单只股票详细分析":
                    ...
                    if "error" in result:
                        ...
                        with st.expander(f"📊 {stock_name} - 评分: {result.get('score', 'N/A')}"):
                            ...
                            with col1:
                                ...
                                st.metric("涨跌幅", f"{result.get('change_percent', 'N/A')}%")
                ...
                if results:
                    ...
                    if results_data:
                        ...
                        with col4:
                            buy_recommendations = len(results_df[results_df['推荐'].str.contains('推荐', na=False)])
                            st.metric("推荐股票", buy_recommendations)
                        
                        # AI Analysis for top stocks
                        if include_ai_analysis:
                            st.subheader("🤖 AI智能分析 - 高分股票")
